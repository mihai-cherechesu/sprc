...
api_prog_1(char *host)
{
	...
	rpc_msg_  load_1_arg = {
		(err_type) SUCC,
		{ { (msg_type_) REQ, { session_key, { (op_type) LOAD, { .r2 = { 2, sdl } } } } } }
	};
	...
	if (result_4->err)
		...
		printf("----> The received values: \n");
		for (int i = 0; i < sdl_len; i++) {
			printf("Sensor %d: ", sdl[i].sensor_id);

			for (int j = 0; j < sdl[i].values.values_len; j++) {
				printf("%f ", sdl[i].values.values_val[j]);
			}
			printf("\n");
		}
	}
